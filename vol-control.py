...
class OSVController(Thread):
    ...
    def __init__(self):
        ...
        self.START_BITS = [0xD0, 0x40, 0x18, 0x06]
        # from datasheet, bits to read flow
        self.FLOW_BITS = [0xD0, 0x51, 0x2C, 0x07]
        ...
        self.bus.write_byte_data(self.FLOW_SENSOR_ADDRESS, 0x0B, 0x00)
        logging.info('  Done')
        '''
    ...
    def calcVolume(self):
        ...
        self.bus.write_i2c_block_data(
            self.FLOW_SENSOR_ADDRESS, 0x00, self.START_BITS)

        # Tell the sensor we want to read the flow value [D051/D052] => Read Compensated Flow value
        self.bus.write_i2c_block_data(
            self.FLOW_SENSOR_ADDRESS, 0x00, self.FLOW_BITS)
        ...
        # Do the conversion
        rd_flow = ((i - 1024.0) * self.FLOW_SENSOR_RANGE /
                   60000.0)  # convert to [L/min](x10)
        return rd_flow
    ...
    def calcPressure(self):
        ...
        answer = self.bus.read_word_data(
            self.PRESSURE_SENSOR_ADDRESS, self.INIT_SIGNAL)

        # bit  shift to get the full value
        answer = float((((answer & 0x00FF) << 8) + ((answer & 0xFF00) >> 8)))
        pressure = (answer-self.SENSOR_COUNT_MIN)*(self.SENSOR_PRESSURE_MAX - self.SENSOR_PRESSURE_MIN) / \
            (self.SENSOR_COUNT_MAX-self.SENSOR_COUNT_MIN) + self.SENSOR_PRESSURE_MIN
